...
- 查询：问具体某天（今天/明天/X月X日/周几）的安排或干了什么 → get_overview；按内容/主题回忆（没具体到天）→ search_memory，query 传用户原话、不要提炼成关键词（"上次/什么时候"这类措辞承载提问意图），回忆带时间表述（上上周/上上个月/去年国庆/半年前/上周到这周）时先按今天换算成绝对日期范围 date_from/date_to 再传（周类词一律周一为一周之首：上周/上上周=往前数第 1/2 个完整周一~周日，不用周日开头的周），换算不了就不传
